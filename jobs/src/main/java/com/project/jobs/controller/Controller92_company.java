...
public class Controller92_company {
	...
	public String registRecruit(Recruit recruit, Model model,HttpSession session) {
		...
		List<Com_interest> ciList =dao.comInterestByCom_no(recruit.getCom_no());
		System.out.println("컴인터레스트"+ciList);
		for(Com_interest ci :ciList) {
			Mem_push mp = new Mem_push(); 
			mp.setTarget_no(ci.getMem_no());
			mp.setContent("새로운 채용 공고입니다.");
			mp.setRecruit_no(dao.getRecruitNo(com.getCom_no()));
			dao.memPushInsert(mp);
		}
